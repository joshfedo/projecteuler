 ...
 * This one makes me excited. It kind of reminds me of doing my senior cap stone on sudoku
 */
...
for ($i = 16; $i >= 3; $i--) {
	for ($j = 0; $j <= 16; $j++) {
		$r_diag = 0;
		$l_diag = 0;
		$l_diag = $grid[$i][$j] * $grid[$i - 1][$j + 1] * $grid[$i - 2][$j + 2] * $grid[$i - 3][$j + 3];
		if ($j > 3) {
			$r_diag = $grid[$i][$j] * $grid[$i - 1][$j - 1] * $grid[$i - 2][$j - 2] * $grid[$i - 3][$j - 3];
		}
		if ($largest < $l_diag) {
			$largest = $l_diag;
		}
